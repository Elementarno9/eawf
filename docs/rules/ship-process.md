...
<!-- BEGIN EAWF:managed id=ship-process version=1.1 hash=9f4bfe1308f89597 -->
# `ship-process`
...
5. **Tag the release.** Under the ``per-phase`` cadence the post-merge ``.github/workflows/phase-release.yaml`` reads the ``(release=v<X.Y.Z>)`` annotation off the phase-close commit, tags the merge commit, and publishes notes from the PR body -- see ``release-process`` for the pre-flight gate that ``eawf phase close`` enforces.
