...
- **Phase/iter-scoped artifact docs** — ``[P<NN>] docs:`` (or ``[P<NN>-I<NN>] docs:``) for documentation artifacts no single wave owns (closure audits, promoted research / decision / incident briefs). Restricted to ``.ea/artifacts/**``; wave-produced docs use the wave form. A commit that stages at least one artifact may also carry ``.ea/state.json`` and ``.secrets.baseline``: rewriting an artifact re-pins its digest in state, and the baseline tracks state.json line numbers. Without an artifact, those two stay rejected.
- **Out-of-phase** — ``<type>: <summary>`` carrying neither carrier: no bracket prefix and no ``Eawf-Wave`` trailer. Accepted ONLY while no phase is PLANNED or ACTIVE (every phase CLOSED or ARCHIVED, and ``state.current.phase_id`` is ``None``) — e.g. a pre-flight chore after the last phase closes and before the next ``/roadmap propose``. Such a commit advances no wave, so there is nothing for either carrier to name. Rejected while any phase is PLANNED or ACTIVE: the gap between one phase's close and the next phase's activation is where unattributed repair commits used to land, so work there becomes a wave of the next phase. A bare ``state:`` subject keeps the narrower rule and is rejected only while ``state.current.phase_id`` is set, since the bookkeeping that brings a PLANNED phase into being has no wave to name yet.
...
Bare ``[P<NN>]`` is accepted for ``type == 'state'`` (any state-bookkeeping path) and ``type == 'docs'`` (restricted to ``.ea/artifacts/**`` plus the digest companions above); for every other bracketed subject the ``-W<NN>`` suffix remains mandatory.
