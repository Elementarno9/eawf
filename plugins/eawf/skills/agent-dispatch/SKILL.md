...
argument-hint: "<wave-id> [--runtime=<id>] [--headless] [--model=<id>]"
user-invocable: true
...
- `--runtime <id>` — override the `Wave.runtime_preference` ladder head for this dispatch. Default is the ladder head.
- `--headless` — route `next_valid_actions` to the daemon headless spawn path instead of the interactive dispatch. Default false.
- `--model <id>` — thread a spawn-model override into the dispatch command (the daemon spawn path already resolves it via `_resolve_spawn_model`). Default is the ladder-resolved model.
- `--sandbox-profile` is NOT accepted — there is no receiving seam at HEAD, so the flag would be inert; it is deferred to P31.
