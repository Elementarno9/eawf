...
def test_run_tui_online_loop_iterates_until_exit(
    ...
    monkeypatch.setattr(tui_app, "_is_tty", lambda: True)
    # P20-W03 reserved ``b`` for the wave-board view; P20-W11 reserved
    # ``c`` for the config modal. Pick inert keys (``a``/``z``) so the
    # iteration test still validates the loop-until-exit contract.
    keys = iter(["a", "z", "\x1b"])
