...
def test_gate_ignores_python_under_a_watched_golden_dir(
    ...
    ``tests/golden/surfaces/cli/`` holds its conftest and test module beside its
    fixtures, so a directory-prefix match flagged a fix to the generator and
    demanded it be relabelled a refresh.
    """
    repo = _init_repo(tmp_path)
    _write_golden(repo, "tests/golden/surfaces/cli/scenarios/conftest.py", "REGEN = 'a'\n")
    _git(repo, "add", "-A")
    _git(repo, "commit", "-q", "-m", "[P27-W18] test: seed scenarios conftest")
    base = _head(repo)
    _write_golden(repo, "tests/golden/surfaces/cli/scenarios/conftest.py", "REGEN = 'b'\n")
    _git(repo, "add", "-A")
...
def test_gate_still_flags_golden_bytes_beside_python(
    ...
    repo = _init_repo(tmp_path)
    _write_golden(
        repo, "tests/golden/surfaces/cli/scenarios/fresh_repo/state.golden.json", '{"v": 1}\n'
    )
    _git(repo, "add", "-A")
    _git(repo, "commit", "-q", "-m", "[P27-W18] test: seed scenarios golden")
    base = _head(repo)
    _write_golden(
        repo, "tests/golden/surfaces/cli/scenarios/fresh_repo/state.golden.json", '{"v": 2}\n'
    )
    _git(repo, "add", "-A")
