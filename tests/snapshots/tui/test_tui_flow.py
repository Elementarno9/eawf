...
(mode, nav scope + mode, screen-stack depth + top-screen class, modal-overlay
depth, toast count) -- equals a declared ``terminal_state``. It is the
...
* the 7 G1-G7 operator-journey flow specs (:data:`FLOW_SPECS`) each run GREEN
  against the current reskinned surfaces -- this IS the I00 acceptance wiring
  for the flow-gated journeys.
