...
The legacy ``src/eawf/tui/`` Rich tree (and its ``EAWF_TUI_LEGACY=1``
escape hatch) has been removed — ``tui_v2`` is the only TUI surface.
These tests pin the dispatch contract at the boundary
(:func:`eawf.cli.app._dispatch_tui`):

* an interactive TTY launches the Textual :class:`~eawf.tui_v2.app.EaApp`;
* the non-TTY / ``--plain`` / ``--no-input`` path emits the deterministic
  ``tui_v2`` status frame (:func:`eawf.tui_v2.offline.emit_status`);
* the legacy ``eawf.tui`` package is gone (no longer importable).
"""
...
def _stub_dispatch(
    ...
    *,
    ...
    monkeypatch.setattr("eawf.tui_v2.offline.emit_status", fake_emit_status)
...
# --------------------------------------------------------------------------
# Interactive TTY launches tui_v2 (no escape hatch remains).
# --------------------------------------------------------------------------


def test_interactive_launches_tui_v2(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_legacy_env_no_longer_routes_anywhere(monkeypatch: pytest.MonkeyPatch) -> None:
    """``EAWF_TUI_LEGACY=1`` is dead — the TTY path still launches tui_v2."""
...
# --------------------------------------------------------------------------
# Non-TTY / plain / no-input fall back to the tui_v2 status emitter.
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The legacy tree is gone — importing it must fail.
# --------------------------------------------------------------------------


def test_legacy_tui_package_removed() -> None:
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("eawf.tui")
