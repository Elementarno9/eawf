...
``tui_v2`` widget MUST have an Eä theme active for those ``$var``\\ s to
resolve at stylesheet-parse time — exactly as :class:`eawf.tui_v2.app.EaApp`
...
from eawf.tui_v2.theme import EA_THEMES, LOGICAL_THEMES


class PaletteHarnessApp(App[None]):
    """Bare host App with the Eä themes registered + the dark one active.

    Mirrors :class:`eawf.tui_v2.app.EaApp`'s theme bootstrap so a widget
