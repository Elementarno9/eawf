...
:meth:`~eawf.tui_v2.widgets.backlog_table.BacklogTable.apply_filter` and
``/sort backlog`` drives
:meth:`~eawf.tui_v2.widgets.backlog_table.BacklogTable.cycle_sort` on the
mounted backlog pane. The repo scope mounts the ``BacklogTable``, so these
Pilot tests host the real :class:`~eawf.tui_v2.app.EaApp` and observe the
...
from eawf.tui_v2.palette.verbs import _handle_filter, _handle_sort
from eawf.tui_v2.widgets.backlog_table import SORT_KEYS, BacklogTable
