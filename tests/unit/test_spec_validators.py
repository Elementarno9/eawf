...
@pytest.mark.parametrize(
    "scopes,expected",
    [
        (["src/eawf/surfaces/tui/app.py"], True),
        (["src/eawf/surfaces/render/envelope.py"], True),
        (
            [
                "src/eawf/state/models.py",
                "src/eawf/surfaces/tui/widgets/header.py",
            ],
            True,
        ),
...
def test_requires_mockup_reference_treats_whitespace_waiver_as_missing() -> None:
    ...
    assert requires_mockup_reference(
        file_scopes=["src/eawf/surfaces/render/envelope.py"],
        mockup_present=False,
