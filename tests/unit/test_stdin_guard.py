...
blocks indefinitely — :func:`eawf.cli._stdin.require_piped_stdin` exits
...
:mod:`eawf.cli._stdin` rather than ``sys.stdin`` itself — Click's
...
def test_require_piped_stdin_exits_with_hint_when_stdin_is_tty(
    ...
    with (
        patch("eawf.cli._stdin.sys", _SysStub(isatty=True)),
...
def test_require_piped_stdin_returns_silently_when_stdin_is_piped() -> None:
    with patch("eawf.cli._stdin.sys", _SysStub(isatty=False)):
...
def test_require_piped_stdin_quotes_command_name_in_hint(
    ...
    with patch("eawf.cli._stdin.sys", _SysStub(isatty=True)), pytest.raises(typer.Exit):
...
def test_cc_statusline_exits_2_when_invoked_at_tty() -> None:
    with patch("eawf.cli._stdin.sys", _SysStub(isatty=True)):
