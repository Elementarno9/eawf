...
# --------------------------------------------------------------------------
# W28: the ASCII-art Seal -- deterministic accent-on-surface TEXT brand mark
# that the hero surfaces render in place of the raster image (no graphics
# protocol, theme-portable on one accent colour).
# --------------------------------------------------------------------------
...
def test_seal_art_widget_is_static_with_stable_id_and_class() -> None:
    ...
    # and never touches seal_capable / a graphics protocol.
...
def test_seal_art_widget_independent_of_kill_switch(monkeypatch: pytest.MonkeyPatch) -> None:
    # The art is deterministic TEXT, so the EAWF_SEAL_DISABLE kill switch (which
    # gates the RASTER seal) does NOT suppress it -- the hero art renders in CI /
    # snapshots regardless, unlike seal_image_widget.
    monkeypatch.setenv(SEAL_DISABLE_ENV, "1")
    seal_capable.cache_clear()
    assert seal_image_widget() is None  # raster path is killed
