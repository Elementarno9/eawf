...
(a) Read the source file. (b) Grep for call sites on the active branch. (c) Inspect golden fixtures / snapshot tests. (d) Only then quote the behaviour. Design docs and memory notes are hypotheses to verify, never ground truth; when doc and source drift, quote the source.
