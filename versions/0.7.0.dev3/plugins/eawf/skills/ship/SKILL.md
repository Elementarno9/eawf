...
4. **Squash the state tail, then push the long-running feature branch.** Before EVERY push, fold the trailing run of `state:` commits not yet pushed into one: `git reset --soft <last-non-state-commit>`, then a single `[P<NN>] state:` commit. The tree is unchanged, so nothing needs re-testing. The squash rewrites commit hashes, so re-pin next: `eawf wave verify-commits --repair` re-pins every `Wave.commit` the rewrite left stale; fold any state it writes into the squashed commit with `git commit --amend`. Never squash a commit the remote already has.
5. Open the phase PR via `gh pr create`.
6. **PR-review pass.** Read remote review comments via `gh pr view <PR> --comments` (or the inline equivalent). For each actionable finding, append a follow-up wave to the current iter via `eawf roadmap revise --add-wave` (not a new iter — per the `iter-phase-close-timing` rule). Implement the whole batch of findings first, then push once per batch through step 4 — never one push per fix: every push restarts the full CI matrix, and a cancelled run's minutes stay spent. Wait for green CI, re-request review, and repeat per batch until clean.
7. **Bundle close in the final pre-merge commit.** Once CI is green and the review-passed branch is on the remote, emit a single `[P<NN>] state: close iter + phase (audit=<id>)` commit that bundles `eawf iter close P<NN>-I<MM>` + `eawf phase close P<NN>` (no other touched files). When step 4 folds earlier state commits into it, the squashed commit keeps this close subject. The operator merges that commit to end the phase. This is the final instance of the per-close bookkeeping rule the iter followed throughout: After EVERY `eawf wave close`, land the bookkeeping (state.json + the typed stores under `.ea/store/`, e.g. audit/decision/evidence) BEFORE dispatching the next subagent — an inline subagent's checkout can revert uncommitted state, silently dropping the close. For a single wave close that means amending the cherry-picked wave commit — stage those paths and `git commit --amend` — instead of writing a separate state commit; leave `Wave.commit` unpinned so the amended SHA stays derivable. The bare `[P<NN>] state:` commit is reserved for what names no single wave: a claim batch, an iter close, a phase close. The event store (`.ea/store/event.jsonl`) is gitignored — it is the raw-stdout firehose; never `git add -f` it.
8. **Post-close work goes to the next phase.** Work that surfaces after the phase-close commit is not a bare out-of-phase commit: while any phase is PLANNED or ACTIVE the commit lint rejects a non-state subject with no `Eawf-Wave` trailer. Append it as a wave of the next PLANNED or ACTIVE phase (`eawf roadmap revise <next-phase> --add-wave`) and commit it under that wave's trailer once the phase is active and the wave claimed. Only with nothing PLANNED or ACTIVE does the bare `<type>: <summary>` form pass.
...
- [ ] The trailing `state:` run is squashed and the wave pins re-verified before each push.
