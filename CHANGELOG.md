...
- **The release record tag was not bumped for the new `Release.adoption` field.** Every row this checkpoint writes carries an `adoption` key under the unchanged `release/v1` tag, and `Release` refuses unknown fields, so a reader built before this checkpoint rejects every such row outright rather than misreading it. Corrected after publication: this note first said such a reader could not tell a record written before the field from one written after, and that it misread the field's absence; in fact it cannot load the row at all. The next checkpoint writes `release/v2` and still reads `release/v1`, so the rows committed at this checkpoint stay valid.
...
- **The front door installs end to end and platform support is classified.** The install path a newcomer takes was broken; it is repaired, and each advertised platform now states whether its evidence came from a real host.
...
- **The prose lints no longer flag three classes of non-violation.** EAWF014 (the no-manual-wrap rule) probed a single character for an ordered-list delimiter, so it read the `0` of `10.` as the delimiter and treated every list item past the ninth as wrapped prose. EAWF013 (citation-marker position) enumerated the shapes a reference row may take, so a row naming its source in plain words was reported as a detached marker. EAWF017 (inline-reference tabulation) matched the heading `## References` exactly, so an artifact numbering its sections scanned its whole reference list as body prose. Across the long-term research briefs this clears 255, 28, and 19 findings respectively.
...
- **The retired telemetry prototype is no longer named in the committed record.** Twelve research briefs lose 69 mentions of the upstream project whose dataclasses and pricing table were vendored in; the name becomes "the telemetry prototype" and the private repository URL is removed rather than replaced. No verdict, count, identifier, or citation changes.
- **Lifecycle-id narration is gone from 506 files of comments and docstrings.** 944 id-only parentheticals — the `(W08)` / `(P29-I07-W06)` form that turns a comment into a change log — were removed under the new `comment-economy` rule, which extends rule 25's ban on design-decision provenance to bare lifecycle ids. Only parentheticals containing nothing but ids, with prose before them on the same line, were touched: these comments are hard-wrapped, so a broader sweep spans line boundaries and leaves dangling sentence openers and half-stripped ids. 811 mentions remain, embedded in sentences that need rewriting rather than deleting.
- **The 60-second bound on targeted test runs is stated in the rule that owns wave verification.** `lean-wave-verification` already required targeted tests per wave and the full suite once per iter; it now bounds the targeted selection at 60 seconds, since a slower selection is over-scoped and stops being run. Recorded as an amendment rather than a new rule, so the two obligations cannot drift apart.
...
- **`state.json` `schema_version` advanced 1.15 -> 1.16.** Additive: it adds `RuntimeCarry.counter_resets`, counting the times a wave's counter source reset under it — a truncated transcript, or a change to what the duration measures. A reset drops the runtime measured before it, so the count is the recorded REASON a wave may close with less runtime than it really spent; without it, an honest reset is indistinguishable from a capture path that silently did nothing. It defaults to `0`; the `v1_15_to_v1_16` step backfills it on every existing carry. Run `eawf migrate`; the step is idempotent and replay-safe.
- **`state.json` `schema_version` advanced 1.14 -> 1.15.** Additive: it adds `RuntimeBaseline.session_id` (inherited by `RuntimeLatest`), recording which session's cumulative counters a snapshot was read from, and `Wave.runtime_carry`, which accumulates the runtime a wave already spent in sessions that have since ended. Together they let a wave claimed in one session and closed in another sum its per-session runtimes rather than differencing counters taken against two different origins. Neither field is recoverable for a wave captured before the bump, so the `v1_14_to_v1_15` step backfills both explicitly. Run `eawf migrate`; the step is idempotent and replay-safe.
...
- Phase 10: multi-runtime — `eawf skill render` CLI, `state.mcp_grants` table + grant/revoke CLI, `claude-agent-sdk` dispatch adapter, dual-runtime envelope ship-gate demo.
- Phase 11: workflow ergonomics — `planning.auto_plan` + `flow.auto_accept` config defaults, SKILL.md bodies prompting plan-mode + AskUserQuestion gates, P11 skill registry tests.
- Phase 12: backlog cluster — decision graph render (B029), memory GC
  + tiered memory (B030+B037), doc-drift linter (B031), PR-body renderer + project wiki (B032+B038), skill eval harness with golden envelopes (B033), sandbox/permission policy table + CLI (B034), file-impact graph (B039).
...
- `README.md`: dropped "(hooks deferred to v0.2 — see B015)" parenthetical from the local-marketplace install section.
...
- Phase 3: instrument probe + `eawf doctor` skeleton, profile composition (11 profile bodies), AGENTS.md + CLAUDE.md rendering with managed regions, init wizard (interactive + `--no-input`), workspace + repo + clone-repo commands, render-output envelope JSON ⇄ markdown, `eawf sync` + extended doctor checks.
- Phase 4: skill engine + typed envelope header/footer + per-skill body schemas, six core skill subclasses (research/prep/audit/ship/review/polish), four meta skills (init/roadmap/differentiate/flow), hook router + runner
  + Claude payload translator, `eawf plugin install/update/doctor claude`, `eawf cc statusline`, `eawf skill list/run` with in-process registry.
- Phase 5: worktree CLI (create/list/merge-back/cleanup), `flow resume` with drift detection, memory authoritative + sync views, MCP install/ update/remove with `owner=eawf` gate, `plan show` CLI + plan_view renderer + JSON Schema export, questionary+rich wizard (replacing Textual), framework self-apply via `eawf init`, AGENTS.md framework rules ported to profile render_blocks.
...
- JSON-injection escape, except-parens, count drift in plugin packager (P06 W05 review).
