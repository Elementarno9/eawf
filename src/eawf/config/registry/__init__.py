...
API so every ``from eawf.config.registry import X`` import keeps working
unchanged. The split is purely structural — responsibility-scoped
submodules behind a stable facade:

- :mod:`~eawf.config.registry.config_keys` — :class:`ConfigKey`,
  :data:`CONFIG_REGISTRY`, and the menu-surface accessors.
- :mod:`~eawf.config.registry.coercion` — raw-answer coercion +
  validation (:func:`coerce_and_validate`, :func:`is_known_key`).
- :mod:`~eawf.config.registry.leaf_keys` — the :class:`LeafKey` model +
  layer-writability shorthands.
- :mod:`~eawf.config.registry.leaf_catalog` — the ~150-row leaf-key data
...
from eawf.config.registry.coercion import coerce_and_validate, is_known_key
from eawf.config.registry.config_keys import (
    CONFIG_REGISTRY,
    ConfigKey,
    ConfigKeyType,
    keys_for_tab,
...
from eawf.config.registry.leaf_catalog import (
    LEAF_KEY_REGISTRY,
    is_known_leaf_key,
    leaf_key_lookup,
    leaf_keys_by_domain,
)
from eawf.config.registry.leaf_keys import LeafKey, LeafKeyType
