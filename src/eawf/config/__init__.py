...
- :func:`eawf.config.layered.merge_config` returns ``(merged, source_map)``.
- :func:`eawf.config.profile.enable_profile` writes a profile to a layer file
  and materialises any required state-keys.
- :data:`eawf.config.defaults.BUILT_IN_DEFAULTS` is the read-only base layer.
- :func:`eawf.config.loader.load_yaml_layer` parses a single layer file.
"""
