...
- :class:`~eawf.hooks.event.HookEvent` — typed Pydantic model with
...
- :class:`~eawf.hooks.event.HookEventType` — frozen v1 enum of every
  ...
  §3.3.
- :class:`~eawf.hooks.runner.HookRunner` — registers callables under one
  or more event types and runs them on demand, returning a list of
  :class:`~eawf.hooks.runner.HookResult` records and never propagating
...
adapter on top via :mod:`eawf.runtimes.claude.hooks_router`.
"""
