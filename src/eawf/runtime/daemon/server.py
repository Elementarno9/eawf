...
# The ``daemon.*`` liveness handlers must be registered before the first
# ping, because that is the one call that must not wait for the deferred
# surface to import. Nothing here references the module by name, so the
# import is the registration and the suppression says so. ``event`` and
# ``state_subscribe`` register as a side effect of the name imports below.
# Every other method module registers on first need; see
# ``ensure_all_methods_registered``.
import eawf.runtime.daemon.methods.daemon  # noqa: F401  — registers daemon.ping/status
