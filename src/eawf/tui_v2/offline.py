"""Deterministic, non-interactive text renderers for the ``tui_v2`` surface.

The interactive Textual app (:mod:`eawf.tui_v2.app`) only runs at a TTY.
...
Both renderers reuse the canonical brand literal (:data:`eawf.render.brand.BRAND_LITERAL`)
and the typed-state breadcrumb (:func:`eawf.tui_v2.widgets.header.build_breadcrumb`)
...
``src/eawf/tui/`` tree (its ``run_tui`` offline mode + its workspace
``offline_render``); the legacy Rich-Layout renderers are gone and
``tui_v2`` owns these paths.
"""
...
from eawf.tui_v2.state_binding import load_state
from eawf.tui_v2.widgets.footer import DEFAULT_HINTS, format_hints
from eawf.tui_v2.widgets.header import DEFAULT_PROJECT_CODE, build_breadcrumb
...
#: segment, matching :func:`eawf.render.brand.render_breadcrumb_head`.
_BRAND_GAP: str = "  "
