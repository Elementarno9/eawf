...
:class:`~eawf.tui_v2.scopes.ScopeScreen`), an :class:`~textual.widgets.Input`
...
* The verbs shown are :func:`~eawf.tui_v2.palette.verbs.visible_verbs`
  ...
  :func:`~eawf.tui_v2.palette.verbs.rank_verbs` re-orders by the typed
  filter.
* ``Tab`` autocompletes the input to the highlighted option's verb name.
* ``Enter`` parses ``(verb, args)`` via
  :func:`~eawf.tui_v2.palette.verbs.split_verb_args`, dismisses the
...
class CommandPalette(ModalScreen[None]):
    ...
    Opened by the shared :class:`~eawf.tui_v2.scopes.ScopeScreen`
    ...
    def _match_verb(self, verb_name: str) -> PaletteVerb | None:
        ...
        Returns:
            The matching :class:`~eawf.tui_v2.palette.verbs.PaletteVerb`,
...
def open_palette(app: App[None]) -> None:
    ...
    The shared :class:`~eawf.tui_v2.scopes.ScopeScreen` ``open_palette``
