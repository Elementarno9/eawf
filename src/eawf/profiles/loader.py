...
:mod:`eawf.profiles.discovery` for the search-order helpers. P25-W15 adds
the explicit composition loader :func:`load_composed_profile` which
discovers, validates, then composes a deterministic
:class:`~eawf.profiles.models.ComposedProfile` from a list of profile ids.
...
:func:`eawf.profiles.compose.compose`.
...
from eawf.profiles.compose import ConflictResolution, compose
from eawf.profiles.discovery import (
    list_profiles_all,
    load_profile_with_discovery,
)
from eawf.profiles.models import ComposedProfile, ProfileBody
...
def load_composed_profile(
    ...
    *,
    ...
    :mod:`eawf.profiles.compose`. With ``conflict_resolution="fail"``
    (default — V3 fail-fast) an undeclared conflict raises
    :class:`~eawf.profiles.compose.ProfileConflict`; ``"first-wins"``
    ...
    Args:
        ...
        conflict_resolution: ``"fail"`` (default) raises
            :class:`~eawf.profiles.compose.ProfileConflict` on undeclared
