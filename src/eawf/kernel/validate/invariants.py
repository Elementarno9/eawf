...
:mod:`eawf.kernel.state.models`; this module covers the rules from
...
class ValidationIndex:
    ...
    :class:`~eawf.kernel.state.models.State` per invariant. Building the groupings
    once per :func:`~eawf.kernel.validate.strict.validate_state` call collapses each
...
def check_closure_rules(state: State, index: ValidationIndex | None = None) -> Iterable[Violation]:
    ...
    Args:
        ...
        index: Pre-built parent->children groupings. When ``None`` (e.g. a
            direct caller outside :func:`~eawf.kernel.validate.strict.validate_state`)
...
def check_mcp_grant_server_ref(state: State) -> Iterable[Violation]:
    ...
    Each :class:`~eawf.kernel.state.models.McpGrant` carries a ``server_id`` whose
