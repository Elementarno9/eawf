...
def doctor_plugin(target_dir: Path) -> DoctorReport:
    ...
    for spec in SKILL_REGISTRY:
        _classify_entry(
            _skill_target(target_dir, spec),
            region_id=f"plugin.codex.skill.{spec.skill_name}",
            kind="skill",
            expected_body=_render_skill(spec).encode("utf-8"),
            ok=ok,
            drifted=drifted,
            missing=missing,
        )
    for spec in AGENT_REGISTRY:
        _classify_entry(
            _agent_target(target_dir, spec),
            region_id=f"plugin.codex.agent.{spec.role}",
            kind="agent",
            expected_body=_render_agent(spec).encode("utf-8"),
            ok=ok,
            drifted=drifted,
            missing=missing,
        )
    for spec in HOOK_REGISTRY:
        ...
        _classify_entry(
            _hook_target(target_dir, spec),
            region_id=f"plugin.codex.hook.{spec.event_type.value}",
            kind="hook",
            expected_body=render_hook_sh(spec.event_type).encode("utf-8"),
            ok=ok,
            drifted=drifted,
            missing=missing,
        )
