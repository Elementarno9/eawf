...
:class:`~eawf.hooks.event.HookEventType`, and returns a typed
:class:`HookEvent`.
...
through :class:`~eawf.hooks.runner.HookRunner`.
"""
...
def route_claude_payload(claude_payload: dict[str, Any]) -> HookEvent | None:
    ...
    if not isinstance(hook_event_name, str) or not hook_event_name:
        logger.warning(
            f"route_claude_payload: missing or non-string hook_event_name; "
            f"payload keys={sorted(claude_payload.keys())[:8]!r}"
        )
        return None
    ...
    if hook_event_name in _CLAUDE_TO_EAWF:
        ...
        if event_type is None:
            logger.warning(
                f"route_claude_payload: tool-use payload not mapped to an Eä event; "
                f"hook_event_name={hook_event_name!r} "
                f"tool_name={claude_payload.get('tool_name')!r}"
            )
            return None
    else:
        logger.warning(
            f"route_claude_payload: unknown Claude hook_event_name={hook_event_name!r}; skipping"
        )
        return None
