...
from eawf.runtimes.claude.statusline_modules import (
    context_tokens,
    git,
    hooks_plugins,
    mcp_health,
...
def _read_payload_from_stdin() -> dict[str, Any]:
    ...
    try:
        ...
        logger.debug(f"statusline: stdin JSON decode failed: {exc}")
        return {}
    if not isinstance(decoded, dict):
        logger.debug(f"statusline: stdin JSON is not a mapping; got {type(decoded).__name__}")
        return {}
    return decoded
...
def _build_segments(
    ...
    for module in _MODULE_ORDER:
        ...
        try:
            ...
            logger.warning(f"statusline: module {module_name!r} raised: {exc}")
            segments.append(
...
def _read_cached_line(session_id: str) -> str | None:
    ...
    try:
        ...
        logger.debug(f"statusline: cache read failed for {session_id!r}: {exc}")
        return None
    if not raw.strip():
        return None
    try:
        ...
        logger.debug(f"statusline: cache JSON decode failed for {session_id!r}: {exc}")
        return None
    if not isinstance(decoded, dict):
        return None
...
def _write_cached_line(session_id: str, line: str) -> None:
    ...
    try:
        ...
        logger.debug(f"statusline: cache write failed for {session_id!r}: {exc}")
