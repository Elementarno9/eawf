...
:class:`eawf.store.envelope.Envelope` ``payload`` field when the
...
class EventPayload(BaseModel):
    ...
    Lands inside :class:`eawf.store.envelope.Envelope.payload` when the
...
class Event(BaseModel):
    ...
    generic :class:`eawf.store.envelope.Envelope`: ``Event`` is the
