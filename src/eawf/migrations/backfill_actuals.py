...
:class:`~eawf.state.models.ActualSummary` from the open->close wall-clock
...
**pure** transform over a typed :class:`~eawf.state.models.State` — no file
...
def backfill_actuals(state: State) -> tuple[State, int]:
    ...
    :class:`~eawf.state.models.ActualSummary` from its
