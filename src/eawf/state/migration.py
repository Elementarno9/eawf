"""Idempotent state-shape repair migrations for :class:`~eawf.state.models.State`.

eawf has no ``schema_version``-gated state-migration framework analogous to
:mod:`eawf.config.migration` (the ``State`` model is read straight through
:func:`eawf.validate.strict.validate_state` →
