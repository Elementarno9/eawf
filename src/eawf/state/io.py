...
def write_state_unlocked(path: Path, data: dict[str, Any]) -> None:
    ...
    :mod:`eawf.state.writer`; this unlocked variant is needed because the
...
def build_event_envelope(
    *,
    ...
    Returns:
        An unsaved :class:`~eawf.store.envelope.Envelope`.
    """
    from eawf.store.envelope import Envelope
...
def append_event(
    ...
    *,
    ...
    through :func:`eawf.store.append.append_envelope`. The caller already
...
def _validate_or_raise(payload: dict[str, Any]) -> State:
    ...
    Returns:
        The validated typed :class:`~eawf.state.models.State`.
