...
- :mod:`eawf.cli.commands.memory_write` — mutating verbs ``add`` /
  ``promote`` / ``prune`` / ``gc`` / ``tier`` / ``compact``.
- :mod:`eawf.cli.commands.memory_read` — read-only verbs ``list`` /
...
# import failure.
from eawf.cli.commands import memory_read as _memory_read  # noqa: E402, F401
from eawf.cli.commands import memory_write as _memory_write  # noqa: E402, F401
