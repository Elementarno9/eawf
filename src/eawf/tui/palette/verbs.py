...
The registry is the single source the :class:`~eawf.tui.palette.command_palette.CommandPalette`
...
def _handle_audit(app: App[None], args: str) -> None:
    ...
    :class:`~eawf.tui.screens.overlays.audit_running.AuditProgress`
    snapshot for the resolved scope + audit id and routes it through the
    App's modal-cap-aware ``push_modal`` (via
    :func:`~eawf.tui.screens.overlays.audit_running.open_audit_running`).
    ...
    """
    from eawf.tui.screens.overlays.audit_running import AuditProgress, open_audit_running
...
def _handle_roadmap(app: App[None], args: str) -> None:
    ...
    :func:`~eawf.tui.screens.overlays.plan_preview.build_plan_tree` from
    the App's bound state and opens
    :class:`~eawf.tui.screens.overlays.plan_preview.PlanPreviewModal`
    ...
    if sub_verb != "propose":
        ...
        return
    from eawf.tui.screens.overlays.plan_preview import build_plan_tree, open_plan_preview
...
def _handle_metrics(app: App[None], args: str) -> None:
    ...
    *args* and opens
    :class:`~eawf.tui.screens.overlays.metrics.MetricsModal` through the
    modal-cap-aware ``push_modal`` (via
    :func:`~eawf.tui.screens.overlays.metrics.open_metrics`). The six
    ...
    """
    from eawf.tui.screens.overlays.metrics import open_metrics, parse_metrics_args
...
def _handle_pr(app: App[None], args: str) -> None:
    ...
    :func:`~eawf.tui.screens.overlays.pr_list.request_pr_list` (a worker
    around the read-only ``gh pr list --json`` shell-out cached for 60 s) and
    opens :class:`~eawf.tui.screens.overlays.pr_list.PrListModal` from the
    ...
    """
    from eawf.tui.screens.overlays.pr_list import request_pr_list
...
def _handle_config(app: App[None], args: str) -> None:
    ...
    Opens :class:`~eawf.tui.screens.overlays.config_modal.ConfigModal`
    through the modal-cap-aware ``push_modal`` (via
    :func:`~eawf.tui.screens.overlays.config_modal.open_config`). The
    ...
    """
    from eawf.tui.screens.overlays.config_modal import open_config
...
def _handle_events(app: App[None], args: str) -> None:
    ...
    :func:`~eawf.tui.screens.overlays.events.load_recent_events` and
    opens :class:`~eawf.tui.screens.overlays.events.EventsModal`
    through the modal-cap-aware ``push_modal`` (via
    :func:`~eawf.tui.screens.overlays.events.open_events`). The live
...
def _handle_find(app: App[None], args: str) -> None:
    ...
    :class:`~eawf.tui.screens.overlays.detail.DetailModal` for the
    top-ranked entity, resolved through
    :func:`~eawf.tui.screens.overlays.detail.resolve_detail`. The modal
...
def _handle_filter(app: App[None], args: str) -> None:
    ...
    drives :meth:`~eawf.tui.widgets.backlog_table.BacklogTable.apply_filter`.
...
def _handle_sort(app: App[None], args: str) -> None:
    ...
    :meth:`~eawf.tui.widgets.backlog_table.BacklogTable.cycle_sort` to
