...
:class:`~eawf.tui.scopes.ScopeScreen`), an :class:`~textual.widgets.Input`
opens pre-filled with ``/``, and an :class:`~textual.widgets.OptionList`
below lists the verbs from the static registry
(:mod:`eawf.tui.palette.verbs`) filtered + ranked by fuzzy match as the
operator types.

Palette UX:

* The verbs shown are :func:`~eawf.tui.palette.verbs.visible_verbs`
  ...
  :func:`~eawf.tui.palette.verbs.rank_verbs` re-orders by the typed
  filter.
* ``Tab`` autocompletes the input to the highlighted option's verb name.
* ``Enter`` parses ``(verb, args)`` via
  :func:`~eawf.tui.palette.verbs.split_verb_args`, dismisses the
...
class CommandPalette(ModalScreen[None]):
    ...
    Opened by the shared :class:`~eawf.tui.scopes.ScopeScreen`
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        current input via
        :func:`~eawf.tui.palette.verbs.split_verb_args`, then matches
    ...
    def _match_verb(self, verb_name: str) -> PaletteVerb | None:
        ...
        Returns:
            The matching :class:`~eawf.tui.palette.verbs.PaletteVerb`,
...
def open_palette(app: App[None]) -> None:
    ...
    The shared :class:`~eawf.tui.scopes.ScopeScreen` ``open_palette``
