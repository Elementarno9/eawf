...
* :mod:`~eawf.tui.snapshot.pilot_harness` — captures a running
...
* :mod:`~eawf.tui.snapshot.asciinema` — composes an asciinema v2
...
2. **Bare ``eawf`` launches ``tui``.** The interactive bare-command
   ...
   :class:`~eawf.tui.app.EaApp`.
3. **The non-TTY / ``--plain`` / ``--no-input`` fallback uses the
   ``tui`` deterministic status emitter**
   (:func:`eawf.tui.offline.emit_status`); the workspace dashboard
   text frame is rendered by :func:`eawf.tui.offline.offline_render`.
"""

from __future__ import annotations

from eawf.tui.snapshot.asciinema import record_cast, write_cast
from eawf.tui.snapshot.pilot_harness import (
    SNAPSHOT_REGEN_ENV,
