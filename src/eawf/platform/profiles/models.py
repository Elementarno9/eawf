...
Schema v2 adds three new fields on :class:`ProfileBody`:
...
class FloorCheck(BaseModel):
    ...
    The verify spine translates each floor check into a
    ...
    * the **enforcement bit** (:attr:`enforce`) — when true, close
    ...
    Attributes:
        ...
        jury_vendors: The disjoint-vendor panel the band-scoped spec jury
            ...
            deterministic canned-ballot gate is what proves the gate
...
class ProfileBody(BaseModel):
    ...
    ``extends`` records a parent profile id when the body was
    ...
    ``output`` mounts the typed :class:`OutputBlock`, resolved
