...
from eawf.lock import sibling, stale
...
class LockHandle:
    ...
    def heartbeat(self) -> None:
        ...
        logger.debug(f"Heartbeat updated for lock {self.path}")
    ...
    *,
    ...
    while True:
        ...
        if lock_path.exists() and stale.is_stale(lock_path):
            ...
            logger.info(f"Stale lock stolen at {lock_path}")
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to unlock {lock_path}", exc_info=True)
        with contextlib.suppress(Exception):
            fh.close()
        lock_path.unlink(missing_ok=True)
