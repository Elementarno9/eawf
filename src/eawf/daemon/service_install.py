...
- **Windows**: pywin32 ``win32serviceutil`` framework around the
  :class:`eawf.daemon.win_service.EawfdService` subclass.
...
from eawf.daemon.runtime_dir import pid_path, runtime_dir
...
def _enable_windows() -> ServiceEnvelope:
    ...
    shelling out to ``python -m eawf.daemon.win_service install`` so
    ...
    service_module = "eawf.daemon.win_service.EawfdService"
    try:  # pragma: no cover - win32-only branch
