...
:mod:`eawf.daemon.server`; this module exists to mark the method name
...
async def state_subscribe(_ctx: MethodContext, _params: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    raise RuntimeError(
        "state.subscribe must be dispatched via the streaming hook in eawf.daemon.server"
    )
...
async def event_subscribe(_ctx: MethodContext, _params: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    raise RuntimeError(
        "event.subscribe must be dispatched via the streaming hook in eawf.daemon.server"
    )
