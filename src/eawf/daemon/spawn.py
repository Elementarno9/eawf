...
:mod:`eawf.daemon.main` lazily from the CLI side; the import lives
inside the spawn function so importing :mod:`eawf.daemon.spawn` from
...
def _spawn_posix(runtime_dir: Path) -> None:
    ...
    Args:
        runtime_dir: Runtime directory the spawned daemon should bind
            ...
            :func:`eawf.daemon.runtime_dir.runtime_dir` resolver picks
    ...
    os.execve(sys.executable, [sys.executable, "-m", "eawf.daemon.main"], os.environ)
...
def _spawn_windows(runtime_dir: Path) -> None:
    ...
    subprocess.Popen(
        [sys.executable, "-m", "eawf.daemon.main"],
