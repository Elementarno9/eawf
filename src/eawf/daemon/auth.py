...
- **Windows** — DACL + post-connect SID verification, delegated to
  :mod:`eawf.daemon.windows_security` (W02). The dispatcher catches
  :class:`~eawf.daemon.windows_security.WindowsAuthError` and
...
def _verify_win32(pipe_handle: Any, expected_sid: Any | None) -> PeerCredential:
    """Verify a Windows pipe peer's SID via :mod:`eawf.daemon.windows_security`.
    ...
    Raises:
        UnauthorizedError: When the peer SID does not match
            *expected_sid*; the helper's
            :class:`~eawf.daemon.windows_security.WindowsAuthError`
