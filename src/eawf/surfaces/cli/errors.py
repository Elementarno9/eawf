...
class ErrorEnvelope(BaseModel):
    ...
    exit_code: int
    """:mod:`eawf.surfaces.cli.exit_codes` value (0..5)."""
    ...
    """Cause-level :class:`~eawf.surfaces.cli.error_codes.ErrorCode` value layered
...
def build_envelope(
    ...
    *,
    ...
    the precise :class:`~eawf.surfaces.cli.error_codes.ErrorCode` over the bucket
    without changing the exit code.

    Args:
        err: The :class:`CliError` (or domain subclass) instance.
        error_code: Optional cause-level
            :class:`~eawf.surfaces.cli.error_codes.ErrorCode`. Stored as its string
...
def _render_text(env: ErrorEnvelope) -> str:
    ...
    -> ``See <error_code>`` when an :class:`~eawf.surfaces.cli.error_codes.ErrorCode`
    ...
    *,
    ...
    Args:
        ...
        flags: Resolved global flags. ``flags.json_output`` controls the
            text/JSON branch in :func:`eawf.surfaces.cli.output.emit_json_or_text`.
        error_code: Optional cause-level
            :class:`~eawf.surfaces.cli.error_codes.ErrorCode` layered over the
