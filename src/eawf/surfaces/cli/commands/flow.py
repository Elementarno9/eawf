...
- ``run`` and ``run --resume`` route through the skill engine; the
  ...
  as ``eawf skill run`` (see :mod:`eawf.surfaces.cli.commands.skill`).
...
def _exit_for_status(status: str) -> int:
    """Mirror :func:`eawf.surfaces.cli.commands.skill._exit_for_status`."""
...
def _parse_stdin_args(stdin_text: str) -> dict[str, Any]:
    ...
    Mirrors :func:`eawf.surfaces.cli.commands.skill._parse_stdin_args`. Empty
