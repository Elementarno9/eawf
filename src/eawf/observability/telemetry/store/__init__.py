...
:class:`~eawf.observability.telemetry.store.base.AbstractMetricsStore` contract:

- :class:`~eawf.observability.telemetry.store.sqlite_store.SqliteMetricsStore` — the
  always-available default, built on the stdlib ``sqlite3`` module.
- :class:`~eawf.observability.telemetry.store.duckdb_store.DuckDbMetricsStore` — the
...
models in :mod:`eawf.observability.telemetry.models` (see
:data:`~eawf.observability.telemetry.store.base.TABLES`), so the schema cannot drift from
the row shape.
"""
